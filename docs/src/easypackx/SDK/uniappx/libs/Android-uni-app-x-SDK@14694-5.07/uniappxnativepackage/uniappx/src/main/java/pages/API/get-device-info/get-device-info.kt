...
open class GenPagesAPIGetDeviceInfoGetDeviceInfo : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesAPIGetDeviceInfoGetDeviceInfo) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesAPIGetDeviceInfoGetDeviceInfo
            val _cache = __ins.renderCache
            val title = ref("getDeviceInfo")
            val data = reactive(DataType__81(items = _uA<Item__7>()))
            val getDeviceInfo = fun(){
                ...
                keys.sort().forEach(fun(key){
                    val value = res[key]
                    if (value != null) {
                        val item = Item__7(label = key, value = "" + (if ((UTSAndroid.`typeof`(value) == "object")) {
                        ...
                        _cE("view", _uM("class" to "uni-common-mt"), _uA(
                            ...
                            _cE("view", _uM("class" to "uni-padding-wrap"), _uA(
                                _cE("view", _uM("class" to "uni-btn-v"), _uA(
                                    _cE("button", _uM("type" to "primary", "onClick" to getDeviceInfo), "获取设备信息")
                                ))
                            ))
                        ))
                    ))
