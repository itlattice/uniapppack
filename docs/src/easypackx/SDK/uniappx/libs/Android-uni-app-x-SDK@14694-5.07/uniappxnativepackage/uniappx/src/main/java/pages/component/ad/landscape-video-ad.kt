...
open class GenPagesComponentAdLandscapeVideoAd : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesComponentAdLandscapeVideoAd) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesComponentAdLandscapeVideoAd
            ...
            return fun(): Any? {
                return _cE("view", _uM("class" to "container"), _uA(
                    _cE("video", _uM("id" to "myVideo2", "src" to unref(src), "controls" to true, "autoplay" to "false", "class" to "video", "onPlay" to onVideoPlay), _uA(
                        if (isTrue(unref(adVisible))) {
                            _cE("view", _uM("key" to 0, "class" to "ad-container"), _uA(
                                _cE("ad", _uM("adpid" to "1597617406", "class" to "ad-class", "onLoad" to loadFun, "onError" to errorFun, "onClose" to closeAd), null, 32)
                            ))
                        } else {
                            _cC("v-if", true)
                        }
