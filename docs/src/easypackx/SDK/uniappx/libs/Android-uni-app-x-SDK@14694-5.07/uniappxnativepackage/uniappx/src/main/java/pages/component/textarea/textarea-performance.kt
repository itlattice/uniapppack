...
open class GenPagesComponentTextareaTextareaPerformance : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesComponentTextareaTextareaPerformance) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesComponentTextareaTextareaPerformance
            val _cache = __ins.renderCache
            val sharedValue = ref("")
            val sharedDisabled = ref(false)
            fun gen_onInput_fn(e: UniInputEvent) {
                sharedValue.value = e.detail.value
                console.log("textarea value changed:", e.detail.value)
            }
                    ...
                    _cE("view", _uM("class" to "page-container"), _uA(
                        ...
                        _cE("view", _uM("class" to "padding-2 flex-row flex-wrap"), _uA(
                            _cE(Fragment, null, RenderHelpers.renderList(100, fun(index, __key, __index, _cached): Any {
                                return _cE("textarea", _uM("key" to index, "value" to unref(sharedValue), "placeholder" to index.toString(10), "disabled" to unref(sharedDisabled), "onInput" to onInput, "class" to "item padding-2"), null, 40, _uA(
                                    "value",
                                    "placeholder",
                                    "disabled"
                                ))
