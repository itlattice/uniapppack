...
open class GenPagesComponentScrollViewIssue18587 : BasePage {
    ...
    open var data: DataType__6
        get() {
            return unref(this.`$exposed`["data"]) as DataType__6
        }
        set(value) {
            setRefValue(this.`$exposed`, "data", value)
        }
    open var scrollTo: (scrollTopValue: Number) -> Unit
        get() {
            return unref(this.`$exposed`["scrollTo"]) as (scrollTopValue: Number) -> Unit
        }
        set(value) {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesComponentScrollViewIssue18587, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesComponentScrollViewIssue18587
            val _cache = __ins.renderCache
            val data = reactive(DataType__6(scrollEndTriggeredTimes = 0))
            fun gen_scrollTo_fn(scrollTopValue: Number) {
                val scroll = uni_getElementById("scroll") as UniScrollViewElement
                scroll.scrollTop = scrollTopValue
            }
