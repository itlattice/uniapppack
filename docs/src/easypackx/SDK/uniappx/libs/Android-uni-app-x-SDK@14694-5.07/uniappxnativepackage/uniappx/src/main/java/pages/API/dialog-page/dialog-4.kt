...
open class GenPagesAPIDialogPageDialog4 : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesAPIDialogPageDialog4) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesAPIDialogPageDialog4
            val _cache = __ins.renderCache
            val currentPage = getCurrentInstance()?.proxy?.`$page`
            val title = ref("dialog 4")
            onPageShow(fun(){
            ...
            return fun(): Any? {
                return _cE("view", _uM("class" to "dialog-container"), _uA(
                    _cE("view", _uM("class" to "dialog-content"), _uA(
                        ...
                        _cE("button", _uM("class" to "mt-10", "onClick" to openDialog3), " open dialog 3 ")
                    ))
                ))
            }
        }
