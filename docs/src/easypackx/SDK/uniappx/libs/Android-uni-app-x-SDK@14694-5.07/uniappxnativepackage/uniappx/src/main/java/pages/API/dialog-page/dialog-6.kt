...
open class GenPagesAPIDialogPageDialog6 : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesAPIDialogPageDialog6) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesAPIDialogPageDialog6
            val _cache = __ins.renderCache
            val currentPage = getCurrentInstance()?.proxy?.`$page`
            val title = ref("dialog 6")
            onPageShow(fun(){
                console.log("dialog 6 onShow")
                uni_setNavigationBarColor(SetNavigationBarColorOptions(frontColor = "#000000", backgroundColor = "#ff0000", success = fun(res) {
