...
open class GenPagesWarehouseAllocateAddorder : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesWarehouseAllocateAddorder) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesWarehouseAllocateAddorder
            val _cache = __ins.renderCache
            val fromdepart = ref(_uO())
            val todepart = ref(_uO())
            val requireOrder = ref(_uO())
            val isOut = ref(false)
            val goodsList = ref(_uA<allocategoodsinfo>())
            val openIndex = ref(-1)
            ...
            val loadCache = fun(){
                var cacheData = common.getStorage("warehouse_allocate")
                if (cacheData == "") {
                    return
                }
                ...
                goodsList.value = _uA<allocategoodsinfo>()
                for(item in resolveUTSValueIterator(list)){
                    goodsList.value.push(allocategoodsinfo(goodsinfo = item["goodsinfo"] as UTSJSONObject, number = item["number"] as Number, stock = item["stock"] as Number, id = item["id"] as Number, remark = if (item["remark"] == null) {
                        null
                    } else {
                ...
                todepart.value = cacheData["todepart"] as UTSJSONObject
                requireOrder.value = cacheData["requireOrder"] as UTSJSONObject
                isOut.value = cacheData["isOut"] as Boolean
                ...
                common.rmStorage("warehouse_allocate")
            }
            val checkOut = fun(e: Boolean){
                isOut.value = e
            }
            val getData = fun(): UTSPromise<UTSJSONObject?> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        var data: UTSJSONObject = _uO("fromdepart" to fromdepart.value, "todepart" to todepart.value, "requireOrder" to requireOrder.value, "isOut" to isOut.value, "goods" to goodsList.value, "remark" to remarkbox["remark"] as String, "thumb" to remarkbox["file"] as UTSArray<UTSJSONObject>)
                        return@w1 data
                })
            }
            val submit = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        if (fromdepart.value["id"] == null || todepart.value["id"] == null) {
                            common.toast("请选择调出/调入店仓")
                            return@w1
                        }
                        ...
                        data["goods"] = gl
                        data["fromdepart"] = (data["fromdepart"] as UTSJSONObject)["id"]
                        data["todepart"] = (data["todepart"] as UTSJSONObject)["id"]
                        var requestOrderID = (data["requireOrder"] as UTSJSONObject)["id"]
                        if (requestOrderID != null) {
                            data["requireOrder"] = requestOrderID
                        }
                        ...
                        var res = await(request.callapi("warehouse/addallocate", data))
                        common.hideLoading(loading.value)
                        common.toast(res.msg)
                        if (res.code < 0) {
                            return@w1
                        }
                        cache.set("refreshallcate", "ok", 10)
                        var orderID = (res.data as UTSJSONObject)["id"] as Number
                        common.timeToPage("/pages/warehouse/allocate/view?id=" + orderID.toString(), true)
                })
            }
            val draft = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        common.setStorage("warehouse_allocate", data)
                        common.toast("保存草稿成功")
                        setTimeout(fun(){
                            common.back()
                        }
            ...
            val chooseFromDepart = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        fromdepart.value = depart
                })
            }
            val chooseToDepart = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        var depart = await(common.navigatoEvent("/pages/common/choosedepart"))
                        if (depart["name"] == null) {
                            return@w1
                        }
                        todepart.value = depart
                })
            }
            val chooseRequireOrder = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        if (todepart.value["id"] == null || fromdepart.value["id"] == null) {
                            common.toast("请先选择调出/入仓库")
                            return@w1
                        }
                        var order = await(common.navigatoEvent("/pages/common/chooserequireorder?depart=" + todepart.value["id"]))
                        if (order["no"] == null) {
                            return@w1
                        }
                        requireOrder.value = order
                        var res = await(request.callapi("warehouse/getgoodslist", _uO("type" to "chooserequire", "order" to order["id"], "depart" to fromdepart.value["id"])))
                        if (res.code < 0) {
                            common.toast(res.msg)
                            return@w1
                        }
                        ...
                        list.forEach(fun(item){
                            goodsList.value.push(allocategoodsinfo(goodsinfo = item, stock = item["stock"] as Number, number = UTSNumber.from(item["number"] as String), id = item["id"] as Number))
                        }
                        )
                })
            }
            ...
            val addGoods = fun(goods: UTSArray<selectedListType>): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        if (fromdepart.value["id"] == null) {
                            common.toast("请先选择调出仓库")
                            return@w1
                        }
                        console.log("收到商品信息")
                        ...
                        for(item in resolveUTSKeyIterator((res.data as UTSArray<UTSJSONObject>))){
                            goodsList.value.push(allocategoodsinfo(goodsinfo = item["goodsinfo"] as UTSJSONObject, number = item["number"] as Number, id = item["id"] as Number, stock = item["stock"] as Number))
                        }
                })
            }
            ...
            return fun(): Any? {
                ...
                val _component_fui_switch = resolveEasyComponent("fui-switch", GenUniModulesFirstuiUvueComponentsFuiSwitchFuiSwitchClass)
                ...
                return _cE("view", null, _uA(
                    _cV(_component_x_viewtitle, _uM("title" to "创建调拨单"), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                        return _uA(
                    ...
                    _cE("scroll-view", _uM("direction" to "vertical", "style" to _nS(_uM("height" to (unref(pageHeight) + "px")))), _uA(
                        _cE("view", _uM("class" to "tasklist background-white width-full"), _uA(
                                        ...
                                        _cE("view", _uM("class" to "linebox display-flex"), _uA(
                                            ...
                                            _cE("view", _uM("class" to "changbox", "onClick" to chooseFromDepart), _uA(
                                                if (isTrue(unref(fromdepart)["name"])) {
                                                    ...
                                                    _cE("text", _uM("key" to 1, "class" to "text placeholder"), "请选择要调出的店/仓")
                                                }
                                            ))
                                        ))
                                    )
                                ...
                                _cV(_component_x_triangleinput, _uM("height" to 35, "width" to 710), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                    return _uA(
                                        _cE("view", _uM("class" to "linebox display-flex"), _uA(
                                            ...
                                            _cE("view", _uM("class" to "changbox", "onClick" to chooseToDepart), _uA(
                                                if (isTrue(unref(todepart)["name"])) {
                                                    _cE("text", _uM("key" to 0, "class" to "text"), _tD(unref(todepart)["name"]), 1)
                                                } else {
                                                    _cE("text", _uM("key" to 1, "class" to "text placeholder"), "请选择要调入的店/仓")
                                                }
                                            ))
                                        ))
                                    )
                            ...
                            _cE("view", _uM("class" to "lineitem"), _uA(
                                _cV(_component_x_triangleinput, _uM("height" to 35, "width" to 710), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                    return _uA(
                                        _cE("view", _uM("class" to "linebox display-flex"), _uA(
                                            _cE("text", _uM("class" to "title"), "关联要货单"),
                                            _cE("view", _uM("class" to "changbox", "onClick" to chooseRequireOrder), _uA(
                                                if (isTrue(unref(requireOrder)["no"])) {
                                                    _cE("text", _uM("key" to 0, "class" to "text"), _tD(unref(requireOrder)["no"]) + "(" + _tD(unref(requireOrder)["add_date"]) + ")", 1)
                                                } else {
                                                    _cE("text", _uM("key" to 1, "class" to "text placeholder"), "要关联的要货单")
                                                }
                                            ))
                                        ))
                                    )
                            ...
                            _cE("view", _uM("class" to "lineitem"), _uA(
                                _cE("view", _uM("class" to "linebox space-between"), _uA(
                                    _cE("text", _uM("class" to "title"), "直接出库"),
                                    _cV(_component_fui_switch, _uM("class" to "swichclass", "scaleRatio" to 0.8, "color" to "#1678fd", "checked" to unref(isOut), "onChange" to checkOut), null, 8, _uA(
                                        "checked"
                                    ))
                                ))
                            ))
                        ...
                        _cE("view", _uM("class" to "goodslist background-white"), _uA(
                                        ...
                                        _cE("view", _uM("class" to "priceinfo space-between"), _uA(
                                            ...
                                            _cE("view", _uM("class" to "priceinfocontent display-flex", "onClick" to fun(){
                                                ...
                                                _cE("text", _uM("class" to "price"), _uA(
                                                    _cE("text", _uM("class" to "yugu"), "调拨数量"),
                            ...
                            _cV(_component_x_addgoods, _uM("onAddGoods" to addGoods, "selected" to unref(selectList), "departID" to unref(fromdepart)["id"]), null, 8, _uA(
                                "selected",
                                "departID"
                            ))
                        )),
