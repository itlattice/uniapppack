...
open class GenUniModulesIboxsChoosemediaComponentsXuCheckboxGroup : VueComponent {
    constructor(__ins: ComponentInternalInstance) : super(__ins) {
        onCreated(fun() {
            val isForm = this.getParent("xu-form")
            if (isForm) {
                val form = this.xForm as ComponentPublicInstance
    ...
    open var xForm: ComponentPublicInstance? by `$data`
    @Suppress("USELESS_CAST")
    override fun data(): Map<String, Any?> {
        return _uM("i18n" to xConfig.i18n as Tmui4xI18nTml, "vals" to _uA<String>(), "childrens" to _uA<ComponentPublicInstance>(), "xForm" to null as ComponentPublicInstance?)
    }
