...
open class GenUniModulesTmxUiComponentsTmxFormTmxForm : VueComponent {
    ...
    open var id: String
        get() {
            return unref(this.`$exposed`["id"]) as String
        }
        set(value) {
            setRefValue(this.`$exposed`, "id", value)
        }
    ...
    companion object {
        ...
        var setup: (__props: GenUniModulesTmxUiComponentsTmxFormTmxForm, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenUniModulesTmxUiComponentsTmxFormTmxForm
            ...
            val oldFormData = ref(_uO() as Any)
            ...
            var flattenObject: flatFunCall? = null
            flattenObject = fun(obj: UTSJSONObject, prefix: String): UTSJSONObject {
                var flattened: UTSJSONObject = _uO()
                val flatMap = obj.toMap()
                for(aitem in resolveUTSValueIterator(flatMap)){
                    var item = aitem as UTSArray<Any?>
                    var key = item[0]!! as String
                    var value = item[1]
                    if (flatMap.has(key)) {
                        val newKey = if (prefix != "") {
                            "" + prefix + "/" + key
                        } else {
                            key
                        }
                        if (value != null && value is UTSJSONObject) {
                            var kFun = flattenObject!!
                            flattened = UTSJSONObject.assign(_uO(), flattened, kFun(value as UTSJSONObject, newKey))
                        } else {
            ...
            __expose(_uM("id" to ("xFormParent-" + getUid()) as String, "pushAdd" to fun(item: FORM_ITEM) {
                if (isdestry.value) {
                    return
                }
        ...
        var props = _nP(_uM("labelWidth" to _uM("type" to "String", "required" to true, "default" to "100"), "labelDirection" to _uM("type" to "String", "required" to true, "default" to "horizontal"), "labelFontColor" to _uM("type" to "String", "required" to true, "default" to "#333333"), "errorAutoPage" to _uM("type" to "Boolean", "required" to true, "default" to true), "modelValue" to _uM("required" to true, "default" to _uO()), "labelFontSize" to _uM("type" to "String", "required" to true, "default" to "16"), "showLabel" to _uM("type" to "Boolean", "required" to true, "default" to true), "errorAlign" to _uM("type" to "String", "required" to true, "default" to "left"), "rules" to _uM("type" to "Object", "required" to true, "default" to fun(): Map<String, UTSArray<FORM_RULE>> {
            return Map<String, UTSArray<FORM_RULE>>()
        }
        ), "watchValidStatus" to _uM("type" to "Boolean", "required" to true, "default" to false), "modelValid" to _uM("type" to "Boolean", "required" to true, "default" to false)))
