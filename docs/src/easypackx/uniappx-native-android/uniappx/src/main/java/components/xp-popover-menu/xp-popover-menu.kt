...
open class GenComponentsXpPopoverMenuXpPopoverMenu : VueComponent, PopoverPropsMenu {
    ...
    companion object {
        ...
        var setup: (__props: GenComponentsXpPopoverMenuXpPopoverMenu, __setupCtx: SetupContext) -> Any? = fun(__props, __setupCtx): Any? {
            val __expose = __setupCtx.expose
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenComponentsXpPopoverMenuXpPopoverMenu
            ...
            val updatePosition = fun(updateWidth: Boolean): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        ...
                        val _calculateWidthAndAdjustPlacement = calculateWidthAndAdjustPlacement(props.placement, availableLeft, availableRight, popoverRect?.width ?: 0)
            ...
            val createAnimation = fun(isEnter: Boolean){
                uniAnimation?.cancel()
                var transform = ""
                var transformOrigin = transformOriginMap.get(innerPlacement.value)
            ...
            onMounted(fun(){
                nextTick(fun(){
                    pageElement = findPageElement(instance.proxy!!.`$el`)
                    if (pageElement != null) {
                                ...
                                _cE("view", _uM("class" to "xmenulist"), _uA(
                                    _cE(Fragment, null, RenderHelpers.renderList(_ctx.menus, fun(item, index, __index, _cached): Any {
                                        return _cE("view", _uM("class" to "xmenu", "key" to index, "style" to _nS(_uM("borderBottom" to if ((index + 1 == _ctx.menus.length)) {
                                            ...
                                            if (isTrue(item["point"])) {
                                                _cE("view", _uM("key" to 0, "class" to "redpoint"))
                                            } else {
                                                _cC("v-if", true)
                                            }
                                        ), 12, _uA(
