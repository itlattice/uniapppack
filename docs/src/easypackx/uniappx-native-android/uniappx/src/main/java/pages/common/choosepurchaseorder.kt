...
open class GenPagesCommonChoosepurchaseorder : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesCommonChoosepurchaseorder) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesCommonChoosepurchaseorder
            ...
            val back = fun(){
                uni__off("/pages/common/choosepurchaseorder", null)
                common.back()
            }
            ...
            val getList = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        var res = await(request.callapi("warehouse/purchaseorder", _uO("page" to page.value, "keywords" to keywords.value, "status" to status.value)))
                        if (res.code < 0) {
            ...
            val selectdata = fun(data: UTSJSONObject){
                uni__emit("/pages/common/choosepurchaseorder", data)
            }
            onLoad(fun(options){
            ...
            return fun(): Any? {
                ...
                return _cE("view", _uM("class" to "basiclist"), _uA(
                    _cV(_component_x_choosetitle, _uM("title" to "选择进货单", "page" to "/pages/common/choosepurchaseorder", "onBack" to back)),
                                ...
                                _cE("view", null, _uA(
                                    _cE(Fragment, null, RenderHelpers.renderList(unref(dataList), fun(item, index, __index, _cached): Any {
                                        return _cE("view", _uM("class" to "orderitem display-flex background-white", "key" to index), _uA(
                                            _cV(_component_x_purchase_order, _uM("select" to true, "order" to item, "index" to index, "onErWeima" to erWeima, "onClick" to fun(){
                                                selectdata(item)
                                            }
                                            ), null, 8, _uA(
                                                "order",
