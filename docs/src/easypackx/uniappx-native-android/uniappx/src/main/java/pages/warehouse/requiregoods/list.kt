...
open class GenPagesWarehouseRequiregoodsList : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesWarehouseRequiregoodsList) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesWarehouseRequiregoodsList
            ...
            val getDate = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        var res = await(request.callapi("common/getdate", _uO("page" to "warehouse/requiregoods")))
                        dateList.value = res.data as UTSArray<String>
                })
            }
            val getList = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        var res = await(request.callapi("warehouse/requireorder", _uO("date" to dateList.value, "page" to page.value, "keywords" to keywords.value)))
                        if (res.code < 0) {
            ...
            val newOrder = fun(){
                common.goto("/pages/warehouse/requiregoods/addrequiregoods", "slide-in-bottom")
            }
            ...
            onShow(fun(){
                var c = cache.get("refreshrequiregoods")
                if (c == "ok") {
                    loadData()
                    cache.rm("refreshrequiregoods")
                }
            }
            )
            onReady(fun(){
            ...
            return fun(): Any? {
                ...
                return _cE("view", null, _uA(
                    _cV(_component_x_viewtitle, _uM("title" to "要货单"), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                        return _uA(
                            _cE("view", _uM("class" to "handles display-flex"), _uA(
                                _cE("view", _uM("class" to "addbtn display-flex", "onClick" to newOrder), _uA(
                                    _cV(_component_x_iconfont, _uM("height" to 18, "class" to "icon", "name" to "jiahao", "size" to "14", "color" to "white")),
                                    _cE("text", _uM("class" to "name color-white"), "新增")
                                )),
                                _cE("view", _uM("class" to "robot display-flex"), _uA(
                                    _cV(_component_x_iconfont, _uM("height" to 18, "class" to "icon", "name" to "robot", "size" to "18", "color" to "white")),
                    ...
                    _cE("view", _uM("class" to "searchview background-white display-flex"), _uA(
                        ...
                        , "width" to 570, "placeholder" to "要货单号/备注"), null, 8, _uA(
                            "modelValue"
                        )),
                        _cE("view", _uM("class" to "display-flex shaixuan"), _uA(
                            _cV(_component_x_iconfont, _uM("name" to "shaixuan", "class" to "icon", "color" to "#1776FC", "size" to "20", "height" to 35)),
                                                ...
                                                return _cV(_component_x_longpress, _uM("key" to index, "onLongpress" to longPress), _uM("default" to withSlotCtx(fun(): UTSArray<Any> {
                                                    return _uA(
                                                        _cE("view", _uM("class" to "orderitem display-flex background-white"), _uA(
                                                            _cV(_component_x_requiregoods_order, _uM("width" to unref(orderWidth), "order" to item, "index" to index, "onErWeima" to erWeima), null, 8, _uA(
                                                                "width",
                                                                "order",
                                                                "index"
                                                            )),
