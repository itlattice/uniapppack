...
open class GenPagesCommonChooseallocateorder : BasePage {
    ...
    companion object {
        ...
        var setup: (__props: GenPagesCommonChooseallocateorder) -> Any? = fun(__props): Any? {
            val __ins = getCurrentInstance()!!
            val _ctx = __ins.proxy as GenPagesCommonChooseallocateorder
            ...
            val back = fun(){
                uni__off("/pages/common/chooseallocateorder", null)
                common.back()
            }
            val scanCode = fun(code: Any): UTSPromise<Unit> {
                return wrapUTSPromise(suspend {
                        console.log("选择进货单页面收到扫码", code)
                })
            }
            val getList = fun(): UTSPromise<Unit> {
                return wrapUTSPromise(suspend w1@{
                        var res = await(request.callapi("warehouse/allocate", _uO("page" to page.value, "keywords" to keywords.value, "from" to fromdepart.value, "to" to todepart.value)))
                        if (res.code < 0) {
            ...
            onLoad(fun(options: OnLoadOptions){
                var depart = options["todepart"]
                if (!(depart == null || depart == "null")) {
                    todepart.value = depart.toString()
                }
                depart = options["fromdepart"]
                if (!(depart == null || depart == "null")) {
                    fromdepart.value = depart.toString()
                }
                loadData()
            }
            )
            ...
            onShow(fun(){
                ...
                console.log("选择调拨单页面开始监听扫码")
                eventID.value = uni__on("scanCode", scanCode)
            }
            )
            onHide(fun(){
                console.log("选择进货单页面停止监听扫码")
                uni__off("scanCode", eventID.value)
            }
            )
            onUnload(fun(){
                uni__off("/pages/common/chooseallocateorder", null)
                console.log("选择调拨单页面停止监听扫码")
                uni__off("scanCode", eventID.value)
            }
            )
            return fun(): Any? {
                ...
                return _cE("view", _uM("class" to "basiclist"), _uA(
                    _cV(_component_x_choosetitle, _uM("title" to "选择调拨单", "page" to "/pages/common/chooseallocateorder", "onBack" to back)),
                    _cE("view", _uM("class" to "searchview background-white"), _uA(
                        ...
                        , "width" to 710, "placeholder" to "进货单号/备注"), null, 8, _uA(
                            "modelValue"
                        ))
                    )),
                                ...
                                _cE("view", null, _uA(
                                    _cE(Fragment, null, RenderHelpers.renderList(unref(dataList), fun(item, index, __index, _cached): Any {
                                        return _cE("view", _uM("class" to "orderitem display-flex background-white", "key" to index), _uA(
                                            _cV(_component_x_allocate_order, _uM("select" to true, "order" to item, "index" to index, "onErWeima" to erWeima), null, 8, _uA(
                                                "order",
                                                "index"
                                            ))
                                        ))
